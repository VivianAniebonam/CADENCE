  ...
  const handlePostGig = async (e) => {
    ...
    try {
      ...
      if (response.data.msg === "Gig posted successfully") {
        navigate("/gig-board-menu");
      }
    } catch (error) {
      console.error("Post Gig Error:", error.response?.data || error);
      setError(error.response?.data?.msg || "Server Error - Could not post gig.");
    }
  };
      ...
      <div className="post-gig-card">
        ...
        <form onSubmit={handlePostGig}>
          ...
          <div className="input-group">
            ...
            <select value={gigType} onChange={(e) => setGigType(e.target.value)} required>
          ...
          <button type="submit" className="post-gig-btn">Post to Gig Board</button>
        </form>
      </div>
    </div>
  );
